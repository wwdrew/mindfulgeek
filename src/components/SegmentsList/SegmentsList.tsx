...
const isAudioSegment = (segment: AudioFile): segment is AudioSegment => segment.type === 'audio'
...
const SegmentsList = ({ duration }: Props) => {
  ...
  const audioMap = data.map(item => ({
    id: item.id,
    duration: item.type === 'audio' ? item.duration : Math.floor(item.ratio * silenceDuration),
    type: item.type
  }))
  ...
  console.log({fullDuration, audioDuration, silenceDuration, actualSilenceDuration, silenceVariance})
